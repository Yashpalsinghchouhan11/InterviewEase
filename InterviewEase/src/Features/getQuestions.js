...
export const getQuestions = (interview_id, index) => {
    ...
    return new Promise((resolve, reject) => {
        axios
            .get(`http://127.0.0.1:8000/login/get_questions/${interview_id}`, {
            ...
            .then((response) => {
                resolve(response); // Return only the response data
            })
            .catch((error) => {
    ...
    async ({ interviewId, index }, thunkAPI) => {
        try {
            ...
            if (response.status === 200) {
                ...
                thunkAPI.dispatch(setQuestionId(response.data.question_id))
            } 
        } catch (error) {
